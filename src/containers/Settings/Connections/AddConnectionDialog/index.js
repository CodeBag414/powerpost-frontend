...
class AddConnectionDialog extends React.Component {
  ...
  initializeFutureChannels() {
    ...
    if (this.props.subChannels.length) {
      for (let i = 0; i < this.props.subChannels.length; i++) {
        ...
        <div>
          ...
          <div>
            ...
            { this.props.subChannels.connected && this.props.subChannels.connected.map((channel, i) =>
                <ConnectionsListItem key={i} connectionIcons={channelIcons} connection={channel} hidden={ channel.status === "1" ? true : false } subChannel={true} channelType={`${channel.channel} ${subChannelType}`} toggleConnection = {this.toggleConnection} />
              )
            }
            { this.props.subChannels.available && this.props.subChannels.available.map((channel, i) =>
                <ConnectionsListItem key={i} connectionIcons={channelIcons} connection={channel} subChannel={true} hidden={ channel.status === "1" ? true : false } connected={false} channelType={`${channel.channel} ${subChannelType}`} toggleConnection = {this.toggleConnection} />
              )
            }
          </div>
        </div>
        ...
        <Tabs index={this.state.index} onChange={this.handleTabChange}>
          ...
          <Tab label="Wordpress" theme={tabTheme}>
            <div className={styles.tabContent}>
                ...
                <PPButton label="Fetch Blogs" primary={this.state.wordpressPasswordError || this.state.wordpressUrlError || this.state.wordpressUserNameError ? false : true} disabled={ this.state.wordpressPasswordError || this.state.wordpressUrlError || this.state.wordpressUserNameError ? true : false} onClick={this.fetchBlogs} />
                <hr />
                </div>
                }
                { this.props.subChannels.length && this.props.subChannels.map((channel, i) => 
                   <ConnectionsListItem key={i} connection={channel} hidden={false} subChannel={true} toggleConnection = {this.toggleConnection} />
                )}
                </div>
